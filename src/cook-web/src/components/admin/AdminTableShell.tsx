...
const renderEmptyRow = ({
  ...
  return (
    <TableEmpty
      ...
      className='h-40 px-6 py-0 align-middle text-sm text-muted-foreground'
    >
...
export default function AdminTableShell({
  ...
  const hasVisiblePagination = !isEmpty && shouldRenderPagination(pagination);
  const hasVisibleFootnote = !isEmpty && Boolean(footnote);
  const shouldRenderFooter =
    (showFooterWhenLoading || !loading) &&
    Boolean(hasVisibleFootnote || footer || hasVisiblePagination);
