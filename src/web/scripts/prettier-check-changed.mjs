...
function readChangedWebFiles(repoRoot, base, head) {
  ...
  const args = base
    ...
    : [command, '-r', '--name-only', '-z', ...diffTarget, '--', 'src/web'];

  const output = gitOutput(args, repoRoot);
  return output
    .split('\0')
    .filter(Boolean)
    .filter(file => file.startsWith(WEB_PREFIX))
    .map(file => file.slice(WEB_PREFIX.length));
}

function runPrettier(webRoot, files) {
  if (files.length === 0) {
...
const changedFiles = readChangedWebFiles(repoRoot, resolvedBase, head);
process.exit(runPrettier(webRoot, changedFiles));
