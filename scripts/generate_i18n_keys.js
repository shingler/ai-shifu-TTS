...
const WEB_TYPES = path.join(ROOT, 'src', 'web', 'src', 'types');
const OUT_FILE = path.join(WEB_TYPES, 'i18n-keys.d.ts');
...
function writeTypes(keys) {
  if (!fs.existsSync(WEB_TYPES)) {
    throw new Error(`Web frontend types directory not found: ${WEB_TYPES}`);
  }
